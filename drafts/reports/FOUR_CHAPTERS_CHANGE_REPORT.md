...
| `docs/nodes/seed-node.md` | Added the missing public Seed Node role: stable P2P identity, public listener and multiaddr, layered firewall/NAT/DNS setup, external and real-peer verification, monitoring, rotation, and the distinction between community and official/default seeds. |
...
| `docs/nodes/configuration.md` | Tied `.env`, Compose, YAML, genesis, descriptors, and RabbitMQ files to the checked-out official orchestrator. Added current-source discovery for options, environment, services, profiles, service `--help`, and `multiaddr`, plus one-change and upgrade-drift procedures. |
